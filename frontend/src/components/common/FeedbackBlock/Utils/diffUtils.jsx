// Utility functions for text diffing and comparison

export function onlyWrongWords(userAnswer, correctAnswer) {
...
export function onlyMissingWords(userAnswer, correctAnswer) {
  if (!userAnswer && !correctAnswer) return null;
  const userTokens = String(userAnswer || "").split(/\s+/);
  const correctTokens = String(correctAnswer || "").split(/\s+/);
  const len = Math.max(userTokens.length, correctTokens.length);
...
export function diffWithNeutral(userAnswer, correctAnswer) {
  if (!userAnswer && !correctAnswer) return null;
  const userTokens = String(userAnswer || "").split(/\s+/);
  const correctTokens = String(correctAnswer || "").split(/\s+/);
  const len = Math.max(userTokens.length, correctTokens.length);
...
export function diffWithNeutralCorrect(userAnswer, correctAnswer) {
  if (!userAnswer && !correctAnswer) return null;
  const userTokens = String(userAnswer || "").split(/\s+/);
  const correctTokens = String(correctAnswer || "").split(/\s+/);
  const len = Math.max(userTokens.length, correctTokens.length);
